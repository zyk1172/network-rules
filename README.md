...
- `sources/upstreams.json`：登记 MetaCubeX/meta-rules-dat 和 blackmatrix7/ios_rule_script 的公开源、格式、许可证和适配路径；AI 规则只按白名单接入 ChatGPT、Claude 与 Gemini，BlackMatrix7 另外提供 PT 与 Netflix。
...
- `dist/quantumult-x/aggregate.list`：圈 X 的单个远程分流列表入口，内部按 PT、广告、ChatGPT、Claude、Gemini、Telegram、YouTube、Netflix、Apple、Google 等分类分段。

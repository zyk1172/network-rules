...
- [`sources/upstreams.json`](sources/upstreams.json)：schema v2 上游清单，支持 `category → client → 1..N components`，并明确哪些组件进入 canonical model。
...
- Quantumult X 使用一个远程入口：`dist/quantumult-x/aggregate.list`。
...
`.github/workflows/update-rules.yml` 每日 FETCH 上游、构建、验证并创建更新 Pull Request。PR 会附带 `dist/build-report.json` 的摘要；不会自动合并，必须人工审查上游 diff、冲突、patch 生命周期和许可证。
...
本项目原创脚本、架构和文档代码使用 [MIT License](LICENSE)。`sources/` 中登记和 `dist/` 中转换生成的第三方规则数据，仍受各上游项目的许可证、NOTICE 和 attribution 要求约束；不能因为根目录存在 MIT License 就把全部规则数据解释为 MIT。具体来源见 [`sources/ATTRIBUTIONS.md`](sources/ATTRIBUTIONS.md) 和 [`dist/README.md`](dist/README.md)。

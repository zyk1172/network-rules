...
blackmatrix7 的 PT 规则 README 还列出了 ACL4SSR PrivateTracker 与 trackerslist 等数据来源；本项目不再把这些已被上游收录的列表重复接入。该项目的 `rule/` 是按客户端和分类拆分的目录，不能把目录本身当作单一规则文件；当前只按白名单接入 PT 和 Netflix，后续分类仍需逐项评估。`ACL4SSR/ACL4SSR` 和 `loyalsoldier/v2ray-rules-dat` 暂作为参考源，不进入默认生成链，以免许可证和分类重复增加维护成本。
